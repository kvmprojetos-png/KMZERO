      ...
      <div style={{ flex: 1, overflowY: "auto", background: T.fundo, padding: 14 }}>
        {ok ? (
          ...
          <>
            ...
            <div style={{ fontSize: 12, color: T.texto3, textAlign: "center", margin: "8px 0" }}>{fotos.length}/5 fotos</div>
            ...
            {fotos.length > 0 && (
              <div style={{ background: T.infoFundo, borderRadius: 8, padding: 10, marginBottom: 8, fontSize: 11, color: T.infoTexto }}>
        ...
        {fotosFiltradas.length === 0 ? (
          ...
          datasOrdenadas.map(data => (
            <div key={data} style={{ marginBottom: 16 }}>
              <div style={{ fontSize: 11, color: T.texto2, textTransform: "uppercase", letterSpacing: 0.5, fontWeight: 700, marginBottom: 6, padding: "0 4px" }}>
            ...
            {minhasMsgs.map(m => {
              ...
              return (
                <div key={m.id} style={{ background: enviada ? T.infoFundo : T.superficie /* balão enviado: tom informativo; recebido: cartão comum */, borderRadius: 12, padding: "10px 14px", marginBottom: 8, marginLeft: enviada ? 30 : 0, marginRight: enviada ? 0 : 30, boxShadow: T.sombra }}>
                  ...
                  <div style={{ fontSize: 14, color: T.titulo }}>{m.texto}</div>
                </div>
              );
            })}
          </>
      ...
      <div style={{ flex: 1, overflowY: "auto", background: T.fundo, padding: 14 }}>
        ...
        {Object.entries(grupos).map(([cat, itens]) => (
          <div key={cat} style={{ marginBottom: 16 }}>
            <div style={{ fontSize: 11, color: T.texto2, textTransform: "uppercase", letterSpacing: 1, marginBottom: 8, fontWeight: 700 }}>{cat}</div>
            {itens.map(l => (
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", flex: 1 }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", background: T.fundo, padding: 14 }}>
        ...
        <div style={{ background: T.superficie, borderRadius: 12, padding: 12, marginBottom: 12, border: `1px solid ${T.borda}` }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
            <div style={{ fontSize: 12, color: T.texto2, fontWeight: 600 }}>📊 Resumo</div>
          ...
          <div style={{ display: "flex", gap: 12, fontSize: 13 }}>
            <div><b style={{ color: T.titulo }}>{arquivos.length}</b> arquivo(s)</div>
            <div style={{ color: T.texto2 }}>•</div>
            <div><b style={{ color: T.titulo }}>{formatarTamanhoBytes(totalTamanho)}</b> nesta obra</div>
          </div>
        </div>
        ...
        {carregando ? (
          <div style={{ textAlign: "center", padding: 40, color: T.texto2 }}>Carregando anexos...</div>
        ) : arquivosFiltrados.length === 0 ? (
          <div style={{ textAlign: "center", padding: 40, color: T.texto2 }}>
          ...
          arquivosFiltrados.map(a => {
            ...
            return (
              <div key={a.id} style={{ background: T.superficie, borderRadius: 12, padding: 12, marginBottom: 8, border: `1px solid ${T.borda}`, borderLeft: "4px solid " + cat.cor }}>
                <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
                  <div style={{ fontSize: 32 }}>{iconePorTipoArquivo(a.mime, a.nomeOriginal)}</div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontWeight: 700, color: T.titulo, fontSize: 13, marginBottom: 3, wordBreak: "break-word" }}>
                      {a.descricao}
                    </div>
                    <div style={{ fontSize: 10, color: T.texto2, marginBottom: 3, wordBreak: "break-word" }}>
                      📄 {a.nomeOriginal} • {formatarTamanhoBytes(a.tamanho)}
                    </div>
                    <div style={{ fontSize: 10, color: T.texto2, marginBottom: 6 }}>
                    ...
                    <div style={{ display: "flex", gap: 6 }}>
                      <button
                        ...
                        style={{ flex: 1, padding: "6px 8px", background: T.infoFundo, color: BLUE, border: "1px solid " + BLUE + "55", borderRadius: 8, fontSize: 11, fontWeight: 700, cursor: "pointer" }}
                      >
                        👁️ Ver
                      </button>
                      <button
                        onClick={() => baixarArquivo(a)}
                        style={{ flex: 1, padding: "6px 8px", background: T.sucessoFundo, color: T.sucessoTexto, border: `1px solid ${T.sucessoBorda}`, borderRadius: 8, fontSize: 11, fontWeight: 700, cursor: "pointer" }}
                      >
                        ⬇️ Baixar
                      </button>
                      {podeExcluir && (
                        <button
                          ...
                          style={{ padding: "6px 10px", background: T.erroFundo, color: RED, border: "1px solid " + RED + "55", borderRadius: 8, fontSize: 11, fontWeight: 700, cursor: "pointer" }}
                        >
                          🗑️
                        </button>
                      )}
      ...
      <Modal show={modalUpload} title="⬆️ Anexar Arquivo" onClose={() => { if (!progresso.fase) { setModalUpload(false); setArquivoSelecionado(null); setDescricaoUpload(""); } }}>
        {progresso.fase ? (
          ...
          <>
            ...
            {arquivoSelecionado && (
              <div style={{ background: T.infoFundo, borderRadius: 8, padding: 10, marginTop: 6, marginBottom: 10, fontSize: 12, color: T.infoTexto }}>
      ...
      {visualizando && (
        ...
        >
          <div onClick={e => e.stopPropagation()} style={{ background: NAVY, padding: "10px 14px", color: "#fff", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <div style={{ flex: 1, overflow: "auto", display: "flex", alignItems: "center", justifyContent: "center" }}>
            {(visualizando.mime || "").startsWith("image/") ? (
              ...
              <iframe
                ...
                style={{ width: "100%", height: "100%", border: "none", background: T.superficie }}
