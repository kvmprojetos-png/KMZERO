...
import { NAVY, GREEN, RED, LIGHT, labelS, inputS, selS, bigBtn, T } from "../theme.js";
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", flex: 1 }}>
            ...
            <div key={cliente.id} data-test={`cliente-card-${cliente.id}`} style={{ background: T.superficie, borderRadius: 14, padding: 14, marginBottom: 10, boxShadow: "0 1px 6px rgba(0,0,0,0.06)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", gap: 12 }}>
                <div style={{ flex: 1 }}>
                  ...
                  {cliente.cidade && <div style={{ fontSize: 12, color: T.texto2, marginTop: 4 }}>{cliente.cidade}</div>}
                  {cliente.documento && <div style={{ fontSize: 12, color: T.texto2, marginTop: 4 }}>{cliente.documento}</div>}
                  {(cliente.telefone || cliente.email) && (
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 12, marginTop: 8, fontSize: 11, color: T.texto2 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 8 }}>
                  <button data-test={`edit-cliente-${cliente.id}`} onClick={() => abrirEdit(cliente)} style={{ background: "none", border: "none", color: T.titulo, fontSize: 14, cursor: "pointer" }}>✏️ Editar</button>
              ...
              {cliente.observacoes && <div style={{ marginTop: 10, fontSize: 12, color: T.texto2 }}>{cliente.observacoes}</div>}
            </div>
          ))
        )}
      </div>
      ...
      <Modal show={modal} title={editandoId ? "Editar Cliente" : "Novo Cliente"} onClose={() => setModal(false)}>
        ...
        {editandoId && (
          <button data-test="modal-delete-cliente" onClick={() => { confirmar("Excluir este cliente?", () => { onRemover(editandoId); setModal(false); }); }} style={{ width: "100%", padding: 10, background: T.erroFundo, color: RED, border: `1px solid ${RED}33`, borderRadius: 10, fontWeight: 700, cursor: "pointer", fontSize: 12, marginBottom: 8 }}>🗑️ Excluir Cliente</button>
        )}
