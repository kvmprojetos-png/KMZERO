// Icones.jsx — biblioteca única de ícones da moldura do KMZERO (menu lateral, cabeçalho, login).
// Desenhos: Lucide Icons (https://lucide.dev), licença ISC — Copyright (c) 2022 Lucide Contributors,
// portions Copyright (c) 2013-2022 Cole Bemis (Feather). Copiados de lucide-static@1.47.0
// (https://cdn.jsdelivr.net/npm/lucide-static@1.47.0/icons/<nome>.svg), só o miolo do SVG, sem alteração.
// Sem dependência: SVG inline 24x24, traço fino (1.75 por padrão), cor = currentColor.
// A vitrine (index.html) é estática e NÃO importa este arquivo.
//
// Uso: <Icone nome="bell" /> · <Icone nome="hard-hat" tamanho={20} cor={GOLD} titulo="Obras" />
// Sem 'titulo' o ícone é decorativo (aria-hidden); com 'titulo' vira imagem acessível (role="img" + <title>).
...
const NOVOS_NOMES = {
  ...
  "chart-bar-big": "bar-chart-3"
};

// chave = nome no Lucide. Atributos já em camelCase para o React.
export const ICONES = {
...
// Nomes novos do Lucide (renomeados a partir da v0.4xx) apontam para o mesmo desenho.
for (const [novo, antigo] of Object.entries(NOVOS_NOMES)) ICONES[novo] = ICONES[antigo];

export const NOMES_ICONES = Object.keys(ICONES);

// Avisa uma vez por nome desconhecido — no console, sem quebrar a tela.
const avisados = new Set();
function avisar(nome) {
  if (avisados.has(nome)) return;
  avisados.add(nome);
  if (typeof console !== "undefined") console.warn(`[Icones] ícone desconhecido: "${nome}" — verifique o nome (Lucide) em NOMES_ICONES.`);
}

// Substituto visível para nome errado: círculo com "?", para o erro aparecer em desenvolvimento.
const DESCONHECIDO = (
  <>
...
export function Icone({ nome, tamanho = 18, cor = "currentColor", espessura = 1.75, className, style, titulo }) {
  let conteudo = ICONES[nome];
  if (!conteudo) { avisar(nome); conteudo = DESCONHECIDO; }
  return (
    <svg
      viewBox="0 0 24 24"
      width={tamanho}
...
/* Teste rápido (opcional) — cole numa tela qualquer para ver a biblioteca inteira:
import { Icone, NOMES_ICONES } from "../components/Icones.jsx";
export const TesteIcones = () => (
  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(120px, 1fr))", gap: 12, padding: 16 }}>
    {NOMES_ICONES.map((n) => (
      <div key={n} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12 }}>
        <Icone nome={n} tamanho={20} titulo={n} /> {n}
      </div>
    ))}
    <div><Icone nome="nao-existe" /> deve mostrar "?" e avisar no console uma vez</div>
  </div>
);
*/
